...
const metrics = (page, key) =>
  cell(page, key).evaluate((node) => {
    ...
    return {
      ...
      striped: getComputedStyle(fill).backgroundImage.includes('repeating-linear-gradient'),
...
test('March 2026 renders Mon–Fri, six rows, starting on Mar 2 (SPEC §2 grid rule)', async ({ page }) => {
  ...
  await expect(panel(page).locator('.calendar__legend-item')).toHaveText(['green = done ratio', 'blue = today', 'gray = empty']);
  expect(errors).toEqual([]);
});

test('cells show the done ratio as a green striped fill; empty days show the gray tray', async ({ page }) => {
  await seed(page);
  await page.goto('/');

  const threeOfFive = await metrics(page, '2026-03-02');
  expect(threeOfFive.ratio).toBeCloseTo(0.6, 1);
  ...
  expect(allDone.ratio).toBeCloseTo(1, 1);
...
test('"Show weekends" adds Sat/Sun columns and persists across reload', async ({ page }) => {
  ...
  await toggle.check();
  await expect(weekdays(page)).toHaveText(['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']);
  await expect(cells(page)).toHaveCount(42);
  expect((await cellKeys(page))[0]).toBe('2026-02-23');
...
test('re-renders when the document changes underneath (cross-tab storage event)', async ({ page }) => {
  await seed(page);
  await page.goto('/');
  expect((await metrics(page, '2026-03-02')).ratio).toBeCloseTo(0.6, 1);
