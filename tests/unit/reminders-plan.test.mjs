...
const agreed = (extra = {}) => ({ title: 'Task a', rDate: null, lDate: '2026-09-12', placed: false, ...extra });
...
test('a new open task becomes a reminder; a day only when it is placed', () => {
  ...
  assert.deepEqual(state.b, { title: 'Task b', rDate: '2026-09-12', lDate: '2026-09-12', placed: true });
});
...
test('removing the day in Reminders sends a waiting task to today', () => {
  ...
  assert.deepEqual(state.a, { title: 'Task a', rDate: null, lDate: TODAY, placed: false });
});
...
test('a reminder deleted in Reminders deletes its open task, never recreates it; finished history stays', () => {
  const { ops, result, state } = planSync({
  ...
  assert.deepEqual(ops, []);
  assert.deepEqual(result.deletedInReminders, ['a']);
