...
export function mount(container, nextCtx) {
  ctx = nextCtx;
  const { ui, i18n } = ctx;
  const nav = ui.stageNav({ onBack: () => ctx.goTo(3), onNext: () => ctx.goTo(1), nextLabel: i18n.t('nav.backToCalendar'), center: ui.stageKicker(4) });
  boardEl = ui.h('div', {
    class: 'board',
  ...
  root = ui.h('div', { class: 'stage-body' }, ui.stageDayBar(dayNav(ctx)), ui.stageHeader({ stage: 4, title: i18n.t('stage.4.title'), kicker: false }), boardEl, nav);
